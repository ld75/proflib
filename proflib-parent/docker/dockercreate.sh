#!/bin/bash
				...
				echo "
				Options :
						- --create unchiffre: lancer des conteneurs (si unchiffre vide n'en demarre qu'un)
				...
				createNodes() {
					...
					for i in $(seq $min $max);do
						docker run -tid --privileged --publish-all=true -v /srv/data:/srv/html -v /sys/fs/cgroup:/sys/fs/cgroup:ro --name $USER-debian-$i -h $USER-debian-$i debianansible:10
						docker exec -ti $USER-debian-$i /bin/sh -c "useradd -m -p xxxxxxx/yyyyy $USER"
